...
export default function Ledger(): React.JSX.Element {
  ...
  const refresh = useCallback(async (): Promise<void> => {
    const res = await window.api.orders.unpaid(query.trim() || undefined)
    if (res.ok && res.data) {
      setOrders(res.data)
  ...
  // Grouped by whoever the order is traceable by, so one glance shows who owes
  // what across all of their orders.
  ...
  for (const o of orders) {
    ...
    byCustomer.set(key, { due: prev.due + owedOn(o), count: prev.count + 1, search: prev.search })
  }
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Outstanding - Rs {totalDue} across {orders.length} order
              {orders.length === 1 ? '' : 's'}
            </p>
          </div>
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-muted text-left">
              <tr>
                ...
                <th className="h-10 w-28 px-2 font-medium">Date</th>
                <th className="h-10 w-24 px-2 font-medium">Type</th>
                <th className="h-10 px-2 font-medium">Name</th>
                <th className="h-10 w-28 px-2 font-medium">Phone</th>
                <th className="h-10 w-24 px-2 text-right font-medium">Total</th>
                <th className="h-10 w-24 px-2 text-right font-medium">Paid</th>
                <th className="h-10 w-24 px-2 text-right font-medium">Due</th>
              </tr>
            </thead>
            <tbody>
              {orders.map((o) => (
                ...
                >
                  ...
                  <td className="h-12 px-2 text-muted-foreground">{fmtDateTime(o.createdAt)}</td>
                  <td className="h-12 px-2">{TYPE_LABEL[o.orderType] ?? o.orderType}</td>
                  <td className="h-12 max-w-32 truncate px-2" title={o.customerName ?? ''}>
                    {o.customerName ?? ''}
                  </td>
                  <td className="h-12 px-2 text-muted-foreground">{o.customerPhone ?? ''}</td>
                  <td className="h-12 px-2 text-right">Rs {o.total}</td>
                  <td className="h-12 px-2 text-right text-green-700 dark:text-green-400">
                    {o.amountPaid > 0 ? `Rs ${o.amountPaid}` : '-'}
                  </td>
                  <td className="h-12 px-2 text-right font-bold text-amber-700 dark:text-amber-400">
                    Rs {owedOn(o)}
                  </td>
                </tr>
              ))}
              {orders.length === 0 && (
                <tr>
                  <td colSpan={8} className="p-8 text-center text-muted-foreground">
                    {query.trim() ? 'Nothing outstanding for this search' : 'Nothing outstanding'}
                  </td>
                </tr>
              )}
            </tbody>
