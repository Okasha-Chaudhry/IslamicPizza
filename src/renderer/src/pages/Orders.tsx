...
type FilterTab = OrderStatus | 'all' | 'kitchen'
...
// An unpaid order can still be part-paid and its slip already printed, so the
// badge has to say both at once.
function statusLabel(o: OrderWithItems): string {
  if (o.status === 'pending' && o.amountPaid > 0) return `Rs ${owedOn(o)} due`
  if (o.status === 'pending') return o.kitchenPrintedAt ? 'unpaid (kitchen)' : 'unpaid'
  return o.status
}

// 24-hour time is hard to scan at a counter; AM/PM is what staff read.
...
// Whatever identifies this order to the person at the counter.
function whoFor(o: OrderWithItems): string {
  return o.customerName?.trim() || o.customerPhone?.trim() || ''
}
...
export default function Orders(): React.JSX.Element {
  ...
  async function reprint(order: OrderWithItems, kind: 'receipt' | 'kitchen'): Promise<void> {
    ...
    if (kind === 'kitchen' && res.ok) {
      await window.api.orders.markKitchenPrinted(order.id)
      await refresh()
    }
  }
        ...
        <div className="flex-1 overflow-y-auto rounded-md border">
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-muted text-left">
              <tr>
                <th className="h-10 w-28 px-2 font-medium">Order #</th>
                <th className="h-10 w-20 px-2 font-medium">Time</th>
                <th className="h-10 w-24 px-2 font-medium">Type</th>
                <th className="h-10 px-2 font-medium">Name</th>
                <th className="h-10 px-2 font-medium">Address</th>
                <th className="h-10 w-24 px-2 text-right font-medium">Total</th>
                <th className="h-10 w-32 px-2 font-medium">Status</th>
              </tr>
            </thead>
            <tbody>
              {orders.map((o) => (
                ...
                >
                  <td className="h-12 px-2 font-medium">{o.orderNumber}</td>
                  <td className="h-12 px-2 text-muted-foreground">{fmtTime(o.createdAt)}</td>
                  <td className="h-12 px-2">{TYPE_LABEL[o.orderType]}</td>
                  <td className="h-12 max-w-32 truncate px-2 font-medium" title={whoFor(o)}>
                    {whoFor(o)}
                  </td>
                  <td
                    className="h-12 max-w-40 truncate px-2 text-muted-foreground"
                    title={o.customerAddress ?? ''}
                  >
                    {o.customerAddress ?? ''}
                  </td>
                  <td className="h-12 px-2 text-right font-medium">Rs {o.total}</td>
              ...
              <div className="flex items-end gap-2 border-t p-3">
                <div className="flex-1 space-y-1">
                  <label className="text-xs text-muted-foreground">
                    Received now (Rs {owedOn(selected)} due)
                  </label>
                  <Input
                    type="number"
                    min="1"
              ...
              {selected.status === 'pending' && (
                <Button
                  ...
                  onClick={() => {
                    ...
                    cart.setCustomerName(selected.customerName ?? '')
                    ...
                    cart.setServiceCharge(selected.serviceCharge)
